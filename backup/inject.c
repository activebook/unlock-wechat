...
// ObjectTypeInformation structure
typedef struct _OBJECT_TYPE_INFORMATION {
    ...
    ULONG PoolType; // Use ULONG instead of POOL_TYPE to avoid conflicts with system definitions
...
DWORD WINAPI CloseWeChatMutex(LPVOID lpParam) {
    ...
    for (ULONG i = 0; i < handleInfo->HandleCount; i++) {
        ...
        if (IsTargetMutex(dup)) {
            ...
            if (status == 0) {
                found = TRUE;
                MessageBoxA(NULL, "Closed target mutex handle in WeChat process.", "Success", MB_OK | MB_ICONINFORMATION);
                break;
            }
        }
        CloseHandle(dup);
    ...
    if (!found) {
        MessageBoxA(NULL, "Target mutex not found in WeChat process.", "Not Found", MB_OK | MB_ICONWARNING);
    }
